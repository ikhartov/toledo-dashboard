...
} = useFetch<Report[]>(`/api/${route.params.project}/backups`, {
  default: () => []
})

if (backupsError.value) {
    ...
    cell: ({ row }) => {
      return h('div', { class: 'flex gap-8 justify-between sm:justify-end' }, [
        h(UButton, {
          ...
          to: `/${route.params.project}/backups/${row.original.id}`
        }),
        h(UButton, {
...
<template>
  <UiDashboardContent class="pb-24 h-full">
    <UPageGrid id="project-backups" class="h-full">
      <UPageCard
